...
use fp_core::storage::data::{DataPool, RuntimeStorage, Storable};
...
impl TransactionManager {
        ...
        match index.get(&transaction_id) {
            ...
            None => {
                ...
                let address = storage.store(
                    Data::Transaction(transaction.clone()),
                    TRANSACTION_POOL_NAME.to_string(),
                )?;
    ...
    pub fn get_transaction(&self, transaction_id: u32) -> Result<Transaction, String> {
        ...
        let transaction_address = self
            .get_transaction_address(&transaction_id)
    ...
    fn update_transaction_state(
        ...
        let address = self
            .get_transaction_address(&transaction_id)
            .ok_or_else(|| "No address for given transaction".to_string())?;
    ...
    fn handle_discover(&mut self, packet: &DhcpV4Packet) -> Result<(), String> {
        ...
        if self.is_in(xid) {
            return Err(
                "DISCOVER trying to initiate an uncommited transaction, aborting.".to_string(),
            );
        }
                        ...
                        match t.state {
                            ...
                            TransactionState::Waiting(_) => {
                                ...
                                self.update_transaction_state(
                                    xid,
                                    TransactionState::Requested("REQUESTED".to_string()),
                                )?;
                                return Ok(());
                            }
    ...
    fn handle_offer(&mut self, packet: &DhcpV4Packet) -> Result<(), String> {
        ...
        let t = self.get_transaction(xid)?;
        match t.state {
            TransactionState::Bound(_e) => self
                .update_transaction_state(xid, TransactionState::Waiting("WAITING".to_string()))?,
    ...
    pub fn watchout(&mut self) -> Result<(), String> {
        ...
        let outdated_transactions = keys
            .into_iter()
            .filter(|key| {
                let t = *key;
...
mod test {
    ...
    use crate::packet::dhcp_packet::DhcpV4Packet;
    use crate::transactions::manager::{
        Transaction, TransactionManager, TransactionState, ADDRESS,
    };
    use chrono::Duration;
    ...
    const DHCP_REQUEST: [u8; 300] = [
        0x01, 0x01, 0x06, 0x00, 0xaa, 0xed, 0x4e, 0xea, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf8, 0x4d,
        0x89, 0x82, 0x44, 0x2a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ...
        0x35, 0x01, 0x03, 0x37, 0x0c, 0x01, 0x79, 0x03, 0x06, 0x0f, 0x6c, 0x72, 0x77, 0xfc, 0x5f,
        0x2c, 0x2e, 0x39, 0x02, 0x05, 0xdc, 0x3d, 0x07, 0x01, 0xf8, 0x4d, 0x89, 0x82, 0x44, 0x2a,
        0x33, 0x04, 0x00, 0x76, 0xa7, 0x00, 0x0c, 0x0c, 0x4d, 0x42, 0x50, 0x2d, 0x64, 0x65, 0x2d,
        0x53, 0x61, 0x63, 0x68, 0x61, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ];
    ...
    const DHCP_OFFER: [u8; 300] = [
        ...
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x63, 0x82, 0x53, 0x63,
        0x35, 0x01, 0x02, 0x37, 0x0c, 0x01, 0x79, 0x03, 0x06, 0x0f, 0x6c, 0x72, 0x77, 0xfc, 0x5f,
        0x2c, 0x2e, 0x39, 0x02, 0x05, 0xdc, 0x3d, 0x07, 0x01, 0xf8, 0x4d, 0x89, 0x82, 0x44, 0x2a,
        0x33, 0x04, 0x00, 0x76, 0xa7, 0x00, 0x0c, 0x0c, 0x4d, 0x42, 0x50, 0x2d, 0x64, 0x65, 0x2d,
        0x53, 0x61, 0x63, 0x68, 0x61, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ];
    ...
    const DHCP_DISCOVER: [u8; 300] = [
        ...
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x63, 0x82, 0x53, 0x63,
        0x35, 0x01, 0x01, 0x37, 0x0c, 0x01, 0x79, 0x03, 0x06, 0x0f, 0x6c, 0x72, 0x77, 0xfc, 0x5f,
        0x2c, 0x2e, 0x39, 0x02, 0x05, 0xdc, 0x3d, 0x07, 0x01, 0xf8, 0x4d, 0x89, 0x82, 0x44, 0x2a,
        0x33, 0x04, 0x00, 0x76, 0xa7, 0x00, 0x0c, 0x0c, 0x4d, 0x42, 0x50, 0x2d, 0x64, 0x65, 0x2d,
        0x53, 0x61, 0x63, 0x68, 0x61, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ];
    ...
    const DHCP_ACK: [u8; 300] = [
        ...
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x63, 0x82, 0x53, 0x63,
        0x35, 0x01, 0x05, 0x37, 0x0c, 0x01, 0x79, 0x03, 0x06, 0x0f, 0x6c, 0x72, 0x77, 0xfc, 0x5f,
        0x2c, 0x2e, 0x39, 0x02, 0x05, 0xdc, 0x3d, 0x07, 0x01, 0xf8, 0x4d, 0x89, 0x82, 0x44, 0x2a,
        0x33, 0x04, 0x00, 0x76, 0xa7, 0x00, 0x0c, 0x0c, 0x4d, 0x42, 0x50, 0x2d, 0x64, 0x65, 0x2d,
        0x53, 0x61, 0x63, 0x68, 0x61, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ];
    ...
    fn discover_process(
        ...
        assert_eq!(packet_discover.options.message_type(), Some(1));
        {
            ...
            let t: Vec<Transaction> = test_db
                .exec_and_return(
                    "SELECT * FROM Transactions where identifier = :identifier".to_string(),
                    params! {"identifier" => xid},
                )
                .unwrap();
        ...
        {
            {
                let mut manager = manager.lock().unwrap();
                manager
                    .bind_lease(packet_discover.xid, lease.clone())
                    .unwrap();
            }
            let xid = packet_discover.xid;
            sleep(time::Duration::from_secs(1));
            ...
            let l: Vec<LeaseData> = test_db
                .exec_and_return(
                    "SELECT * FROM PendingLeases where id = :id".to_string(),
                    params! {"id" => transaction.pending_lease_address},
                )
                .unwrap();
        ...
        if success {
            ...
            println!("Testing ack handling");
            {
                ...
                let l: Vec<LeaseData> = test_db
                    .exec_and_return(
                        "SELECT * FROM PendingLeases where id = :id".to_string(),
                        params! {"id" => expected_lease_address},
                    )
                    .unwrap();
                assert_eq!(l.len(), 0);
                ...
                let l: Vec<LeaseData> = test_db
                    .exec_and_return(
                        "SELECT * FROM Leases where address = :address".to_string(),
                        params! {"address" => lease.addr().to_string()},
                    )
                    .unwrap();
                let l = l.get(0).unwrap();
            ...
            {
                {
                    ...
                    manager
                        .update_transaction_state(
                            xid,
                            TransactionState::Waiting("WAITING".to_string()),
                        )
                        .unwrap();
                    packet_request
                        .options
                        .set_server_identifier(Some(Ipv4Addr::BROADCAST));
                    manager.handle_input(&packet_request).unwrap();
                }
                let xid = packet_discover.xid;
                sleep(time::Duration::from_secs(1));
