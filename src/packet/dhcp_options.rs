...
fn _parse_ipv4_type(bytes: &[u8]) -> Option<Ipv4Addr> {
    ...
    Some(Ipv4Addr::from(BigEndian::read_u32(&bytes[..4])))
}
        ...
        while !data.is_empty() {
            ...
            match opt_code {
                1 => {
                    ...
                    options.set_subnet_mask(_parse_ipv4_type(&raw_bytes));
                }
                4 => {
                    let raw_bytes: Vec<u8> = data.drain(..len).collect();
                    options.set_time_server(_parse_ipv4_list_type(&raw_bytes));
                }
                5 => {
                    let raw_bytes: Vec<u8> = data.drain(..len).collect();
                    options.set_name_server(_parse_ipv4_list_type(&raw_bytes));
                }
                12 => {
                    let raw_bytes: Vec<u8> = data.drain(..len).collect();
                    options.set_hostname(_parse_string_type(&raw_bytes));
                }
                15 => {
                    let raw_bytes: Vec<u8> = data.drain(..len).collect();
                    options.set_domain_name(_parse_string_type(&raw_bytes));
                }
                26 => {
                    let raw_bytes: Vec<u8> = data.drain(..len).collect();
                    options.set_interface_mtu(Some(BigEndian::read_u16(&raw_bytes)));
                }
                28 => {
                    let raw_bytes: Vec<u8> = data.drain(..len).collect();
                    options.set_broadcast_addr(_parse_ipv4_type(&raw_bytes));
                }
                42 => {
                    let raw_bytes: Vec<u8> = data.drain(..len).collect();
                    options.set_time_server(_parse_ipv4_list_type(&raw_bytes));
                ...
                50 => {
                    let raw_bytes: Vec<u8> = data.drain(..len).collect();
                    options.set_requested_ip(_parse_ipv4_type(&raw_bytes));
                }
                51 => {
                    let requested_lease_time = data.drain(..len).as_slice().to_owned();
                    options
                        .set_lease_time(Some(BigEndian::read_u32(requested_lease_time.as_slice())));
                }
                53 => {
                ...
                54 => {
                    let raw_bytes: Vec<u8> = data.drain(..len).collect();
                    options.set_server_identifier(_parse_ipv4_type(&raw_bytes));
                }
                55 => {
                ...
                58 => {
                    ...
                    options
                        .set_renewal_time(Some(BigEndian::read_u32(renewal_time_value.as_slice())));
                }
                59 => {
                    let rebinding_time = data.drain(..len).as_slice().to_owned();
                    options
                        .set_rebinding_time(Some(BigEndian::read_u32(rebinding_time.as_slice())));
                }
                61 => {
                ...
                252 => {
                    let raw_bytes: Vec<u8> = data.drain(..len).collect();
                    options.set_wpad(_parse_string_type(&raw_bytes));
                }
...
fn _format_ipv4_list(addrs: &[Ipv4Addr]) -> Vec<u8> {
    ...
    addrs
        .iter()
        .map(|x| {
            buf.extend_from_slice(&_format_ipv4(*x));
        })
        .last();
    buf
}
...
impl DhcpOptions {
    ...
    pub fn set_subnet_mask(&mut self, subnet_mask: Option<Ipv4Addr>) {
        self.defined_options.insert(1);
        self.subnet_mask = subnet_mask;
    }

    pub fn router_option(&self) -> Option<&Vec<Ipv4Addr>> {
        self.router_option.as_ref()
    }

    pub fn set_router_option(&mut self, router_option: Option<Vec<Ipv4Addr>>) {
        self.defined_options.insert(3);
        self.router_option = router_option;
    }

    pub fn time_server(&self) -> Option<&Vec<Ipv4Addr>> {
        self.time_server.as_ref()
    }

    pub fn add_time_server(&mut self, time_server: Ipv4Addr) {
    ...
    pub fn name_server(&self) -> Option<&Vec<Ipv4Addr>> {
        self.name_server.as_ref()
    }

    pub fn add_name_server(&mut self, name_server: Ipv4Addr) {
    ...
    pub fn set_broadcast_addr(&mut self, broadcast_addr: Option<Ipv4Addr>) {
    ...
    pub fn set_requested_ip(&mut self, requested_ip: Option<Ipv4Addr>) {
    ...
    pub fn set_lease_time(&mut self, lease_time: Option<u32>) {
    ...
    pub fn set_message_type(&mut self, message_type: Option<u8>) {
    ...
    pub fn set_server_identifier(&mut self, server_identifier: Option<Ipv4Addr>) {
        self.defined_options.insert(54);
        self.server_identifier = server_identifier;
    }

    pub fn parameter_request(&self) -> Option<&Vec<u8>> {
        self.parameter_request.as_ref()
    }

    pub fn add_parameter_request(&mut self, parameter_request: u8) {
    ...
    pub fn set_renewal_time(&mut self, renewal_time: Option<u32>) {
    ...
    pub fn set_rebinding_time(&mut self, rebinding_time: Option<u32>) {
    ...
    pub fn set_client_identifier(&mut self, client_identifier: Option<Vec<u8>>) {
        self.defined_options.insert(61);
        self.client_identifier = client_identifier;
    }

    pub fn log_server(&self) -> Option<&Vec<Ipv4Addr>> {
        self.log_server.as_ref()
    }

    pub fn set_log_server(&mut self, log_server: Option<Vec<Ipv4Addr>>) {
        self.defined_options.insert(7);
        self.log_server = log_server;
    }

    pub fn wpad(&self) -> Option<&String> {
        self.wpad.as_ref()
    }

    pub fn set_wpad(&mut self, wpad: Option<String>) {
        self.wpad = wpad;
    }

    pub fn set_name_server(&mut self, name_server: Option<Vec<Ipv4Addr>>) {
        self.defined_options.insert(5);
        self.name_server = name_server;
    }

    pub fn set_time_server(&mut self, time_server: Option<Vec<Ipv4Addr>>) {
        self.defined_options.insert(4);
        self.time_server = time_server;
    }

    pub fn interface_mtu(&self) -> Option<u16> {
        self.interface_mtu
    }

    pub fn set_interface_mtu(&mut self, interface_mtu: Option<u16>) {
        self.defined_options.insert(26);
        self.interface_mtu = interface_mtu;
    }

    pub fn ntp_servers(&self) -> Option<&Vec<Ipv4Addr>> {
        self.ntp_servers.as_ref()
    }

    pub fn set_ntp_servers(&mut self, ntp_servers: Option<Vec<Ipv4Addr>>) {
...
mod tests {
    ...
    const OPTION_BYTES: [u8; 50] = [
        0x35, 0x01, 0x05, 0x36, 0x04, 0xc0, 0xa8, 0x00, 0xfe, 0x33, 0x04, 0x00, 0x00, 0xa8, 0xc0,
        0x01, 0x04, 0xff, 0xff, 0xff, 0x00, 0x03, 0x04, 0xc0, 0xa8, 0x00, 0xfe, 0x06, 0x14, 0x01,
        0x01, 0x01, 0x01, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x04, 0x04, 0xd0, 0x43, 0xde, 0xde,
        0xc0, 0xa8, 0x00, 0xfe, 0xff,
    ];

    #[test]
    fn options_from_bytes() {
        let options = DhcpOptions::from(OPTION_BYTES.as_slice());
        assert_eq!(options.message_type().unwrap(), 5);
        assert_eq!(
            options.subnet_mask().unwrap(),
    ...
    fn bytes_from_options() {
        ...
        assert_eq!(
            DhcpOptions::from(bytes.as_slice()),
            DhcpOptions::from(OPTION_BYTES.as_slice())
        );
    }
}
